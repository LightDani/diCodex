...
from .logging_utils import configure_logging, get_logger, timed_operation
...
def run() -> None:
    ...
    LOGGER.info(
        "app.start source=%s pipeline_mode=%s",
        args.source,
        args.pipeline_mode,
    )
    ...
    if args.source == "asah":
        ...
        with timed_operation(LOGGER, "asah_capture"):
    ...
    if args.pipeline_mode == "transform":
        with timed_operation(LOGGER, "transform_only"):
            run_transform_job(
                output_dir=OUTPUT_DIR,
    ...
    with timed_operation(LOGGER, "codingcamp_pipeline"):
